    ...
    *,
    ...
    if args.dry_run:
        if args.source == "lowy_institute":
            cli_logger.info(
                "lowy dry-run discovery discovered=%s pages_scanned=%s",
                len(discovered_urls),
            ...
            dry_run_parts = [
                ...
                "discovery=lowy_publications_listing",
                "url_scope=/publications/*",
            ...
            cli_logger.info(
                "sitemap dry-run discovery discovered=%s selected_sitemaps=%s",
                len(discovered_urls),
                discovery_scan_count,
            )
            dry_run_parts = [
                ...
                f"source={args.source}",
                f"discovered_urls={len(discovered_urls)}",
                f"selected_sitemaps={discovery_scan_count}",
                f"limit={args.limit}",
                f"published_since={_format_datetime(published_since)}",
            ]
    ...
    if args.source == "lowy_institute":
        cli_logger.info(
            "lowy discovery completed discovered=%s pages_scanned=%s",
