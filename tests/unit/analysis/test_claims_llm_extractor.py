"""Unit tests for LLM-backed claim extraction behavior."""
...
def _set_openai_key(monkeypatch: pytest.MonkeyPatch) -> None:
    """Set API key for LLM extractor tests needing environment config."""
    monkeypatch.setenv("OPENAI_API_KEY", "test-key")


def test_llm_extractor_builds_claim_candidates_from_llm_substrings() -> None:
    """Map valid LLM claims to chunk offsets and candidate fields."""
    chunk = _chunk(
    ...
    extractor = LLMClaimExtractor(
        config=LLMClaimExtractorConfig(model="gpt-4.1-mini"),
        http_client=client,
    )
    ...
    assert claims[1].start_char > claims[0].end_char
    assert claims[0].extractor_version == "llm-v1"
    assert claims[0].confidence == 1.0
    assert len(client.seen_payloads) == 1


def test_llm_extractor_skips_claims_not_found_in_chunk_text() -> None:
            ...
            "choices": [
                {
                    "message": {
                        "content": (
                            '{"claims": ['
                            '"Government should ban coal immediately."'
                            "]}"
                        ),
                    },
                },
    ...
    extractor = LLMClaimExtractor(http_client=client)
...
def test_llm_extractor_requires_api_key_environment() -> None:
    """Raise clear errors when API key is missing from environment."""
    if "OPENAI_API_KEY" in os.environ:
        del os.environ["OPENAI_API_KEY"]
    chunk = _chunk("Government should ban new coal projects.")
    extractor = LLMClaimExtractor(http_client=StubHttpClient({}, []))
